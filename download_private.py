...
def get_download_links_from_huggingface(text_only=False):
    base = "https://huggingface.co"
    page = f"/api/models/Leul78/meetplacesales/tree/main"
    cursor = b""
    ...
    while True:
        ...
        headers = {'Authorization': f'Bearer {"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"}'}
        ...
        for i in range(len(dict)):
            ...
            if any((is_pytorch, is_safetensors, is_pt, is_ggml, is_tokenizer, is_text)):
                ...
                if is_text:
                    links.append(
                        f"https://huggingface.co/Leul78/meetplacesales/resolve/main/{fname}"
                    )
...
def start_download_threads(
    file_list, output_folder, start_from_scratch=False, threads=1
):
    thread_map(
...
def download_model_files(links, sha256, output_folder, start_from_scratch=False, threads=1
):
    # Creating the folder and writing the metadata
    if not output_folder.exists():
        output_folder.mkdir()
    with open(output_folder / "huggingface-metadata.txt", "w") as f:
        f.write(f"url: https://huggingface.co/Leul78/meetplacesales/resolve\n")
...
if __name__ == "__main__":
    ...
    base_folder='models'
    output_folder=Path(base_folder) / "meetplacesales"
    download_model_files(links, sha256, output_folder, threads=1
        )
